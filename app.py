...
# 核心 10 大職能介入方案資料庫 (完全比照上傳之 text 內容，並融入 Cole 7 步驟指引與 IRM 模式)
OT_PROGRAMS = {
    "方案一：心情色彩研究室": {
        ...
        "cole_steps": (
            "1. Introduction (導入): 營造安全不具評價的氛圍，介紹流動畫材料特質。\n"
            "2. Activity (活動): 選擇代表情緒的2-3種顏色進行創作，體驗刮、吹等動作控制。\n"
            "3. Sharing (分享): 展示作品並為其命名，分享過程中最舒服或意外的部分。\n"
            "4. Processing (處理): 引導表達在「無法控制顏料流動」時的內在焦虑與放鬆感受。\n"
            "5. Generalizing (概化): 整理並總結非語言創作與情緒外化對於情緒調節的效益。\n"
            "6. Application (應用): 探討當生活情緒難以口語表達時，如何以顏色、音樂或動作整理自己。\n"
            "7. Summary (總結): 感謝成員的開放分享，重申情緒接納與非語言宣洩管道價值。"
        ),
        "irm_mode": "同理模式 (Empathizing) & 鼓勵模式 (Encouraging)：完全接納個案的色彩選擇，肯定其面對不確定創作結果的勇氣，不予以美醜評判。",
        "mechanism": "• 感官與動作調節。\n• 非語言情緒表達。\n• 接受無法完全控制的結果。\n• 經驗選擇與完成感。",
        "grading": (
            "• 簡易：限制三種顏色，提供步驟示範。\n"
    ...
    "方案二：我的安心飲品實驗室": {
        ...
        "cole_steps": (
            "1. Introduction: 說明調配飲品的多重感官設計，建立正念品嚐的安全預期。\n"
            "2. Activity: 依視覺化食譜比例清洗、調配、攪拌，完成個人化飲品。\n"
            "3. Sharing: 為飲品命名，向成員介紹調配概念與希望帶來的身心感受。\n"
            "4. Processing: 口頭引導個案表達品嚐前三口時，口中本體覺與味覺帶來的生理放鬆感。\n"
            "5. Generalizing: 歸納透過依序完成任務，轉移反芻思考、調降過度喚醒之學理。\n"
            "6. Application: 討論如何將本活動設計為下課、下班或睡前轉換的生活安定儀式。\n"
            "7. Summary: 總結多感官調節對於日常健康管理與職能平衡的重要價值。"
        ),
        "irm_mode": "指導模式 (Instructing) & 合作模式 (Collaborating)：提供清晰的視覺化步驟指引，並與個案共同協商、調配最適合其當前需要的感官食材。",
        "mechanism": "• 多感官覺察。\n• 任務順序與注意力集中。\n• 選擇與控制感。\n• 建立健康的自我照顧儀式。",
        "grading": (
            "• 簡易：使用預先分裝材料及圖片食譜。\n"
    ...
    "方案三：把怒氣揉成形": {
        ...
        "cole_steps": (
            "1. Introduction: 說明黏土阻力媒介能提供肌肉出力與張力釋放，建立安全揉捏規範。\n"
            "2. Activity: 以黏土呈現「怒氣出現的造形」，進行揉、壓、搥打，並將其改造成實用容器。\n"
            "3. Sharing: 展示改造後的作品，說明加入了什麼「暫停」或「保護」的防護元素。\n"
            "4. Processing: 表達在強力揉壓黏土時，手部肌肉緊繃到放鬆的身體張力釋放感受。\n"
            "5. Generalizing: 總結將無形憤怒「外化具體化」並改造成功能性器皿的臨床轉化機制。\n"
            "6. Application: 討論在現實生活中遭遇衝突時，如何利用抗阻運動（如推牆）安全降溫。\n"
            "7. Summary: 重申憤怒是需要被安全容納與轉化的能量，讚許成員的具體創作。"
        ),
        "irm_mode": "問題解決模式 (Problem-solving) & 同理模式 (Empathizing)：同理其內在憤怒，但不加以說教，引導其透過實體揉捏作品，主動發掘解決情緒衝動的物理替代方案。",
        "mechanism": "• 本體覺與觸覺調節。\n• 衝動延宕。\n• 具體化與轉化情緒.\n• 問題解決及挫折耐受。",
        "grading": (
            "• 簡易：完成單一情緒造形。\n"
    ...
    "方案四：煩惱分類收納盒": {
        ...
        "cole_steps": (
            "1. Introduction: 引導覺察內在焦慮如何因大腦負荷過重而混亂，說明收納分類概念。\n"
        ...
        "irm_mode": "指導模式 (Instructing) & 合作模式 (Collaborating)：結構化地示範分類規則，並協同個案將看似龐大的職能障礙拆解成當下可執行的一小步。",
        "mechanism": "• 外化反覆思考。\n• 組織與分類。\n• 降低認知負荷。\n• 將焦慮轉化為可執行行動。"
    },
    "方案五：一份給自己的能量餐": {
...
st.write("本系統完全依據 **《AI 情緒調節職能治療引擎》**、**AOTA OTPF-4 活動分析指引**、**Cole 7 步驟團體帶領架構** 與 **Taylor IRM 關係模式 (Therapeutic Use of Self)** 設計，專為職能治療系學生量身打造的方案設計鷹架系統。")
st.markdown("---")
...
uploaded_file_names = []
if uploaded_files:
    st.sidebar.success(f"📈 成功導入 {len(uploaded_files)} 個臨床擴充指引！")
    for uploaded_file in uploaded_files:
        try:
            file_content = uploaded_file.read().decode("utf-8")
            cloud_reference_text += f"\\n\\n=== 雲端即時文獻：{uploaded_file.name} ===\\n{file_content}"
            uploaded_file_names.append(uploaded_file.name)
            st.sidebar.text(f"📄 {uploaded_file.name} (已動態載入)")
        except Exception as e:
...
if target_group:
    ...
    with st.sidebar.expander(f"🔍 查看 {target_group} 詳情"):
        ...
        if "cole_steps" in prog:
            st.markdown(f"**👣 Cole 7步驟團體引導：**\n{prog['cole_steps']}")
        else:
            st.markdown(f"**👣 標準實施步驟：**\n{prog['steps']}")
        if "irm_mode" in prog:
            st.markdown(f"**💓 IRM 治療性自我 (Use of Self)：**\n*{prog['irm_mode']}*")
...
if st.button("🚀 生成標準化職能活動企畫與 AOTA 評量分析"):
    ...
    with col_a:
        ...
        st.markdown(f"### 📋 【Cole 7 步驟團體引導指引與治療性自我 (Use of Self)】")
        st.markdown(f"**• IRM 治療關係溝通模式 (Use of Self)**：*{selected_act['irm_mode'] if 'irm_mode' in selected_act else '以同理與鼓勵建立治療關係'}*")
        
        cole_steps_text = selected_act['cole_steps'] if 'cole_steps' in selected_act else selected_act['steps']
        st.markdown(f"**• Cole 7 Steps 團體動力分析**：\n{cole_steps_text}")
...
* A (Assessment) 臨床評估:
個案因職能角色負荷與焦慮反覆思考，干擾其「{impacted_occupation}」之職能投入。本次介入連結「{theory_link}」架構與 Cole 7 步驟方案，提供「{target_program}」作為調節媒介。治療師採取 IRM 之「{selected_act['irm_mode'].split('：')[0] if 'irm_mode' in selected_act else '支持'}」與其互動。
...
st.write("本對話模組結合 **Cole 7 步驟**與 **IRM 關係模式(治療性自我)** 進行決策推理。若有拖曳上傳之臨床新指引，AI 督導將會動態提取該資料進行對話與微調。")
...
def generate_clinical_aota_fallback(query, uploaded_docs):
    ...
    rag_feedback = ""
    if uploaded_docs:
        rag_feedback = f"\n\n* 雲端 RAG 實時提取成功: 偵測到您上傳了實證文獻 `{uploaded_docs}`。AI 已動態掃描檔案，並將其中的介入標準融入 AOTA 與 Cole 7 步驟分析中：\n"
        for doc in uploaded_docs:
            if "生氣" in doc.lower() or "情緒" in doc.lower() or "cole" in doc.lower():
                rag_feedback += f"- *已將【{doc}】中的團體動力學 7 步驟、治療性運用自我、引導技巧，與 10 大方案整合進行分析。*\n"
            else:
                rag_feedback += f"- *已將【{doc}】中的特定環境限制與個案特徵，作為 AOTA 表現技能 (Performance Skills) 的調適參數。*\n"
    else:
...
2. **治療性自我 (Therapeutic Use of Self)**：
   - 根據個案特性，選擇適合 the IRM 溝通模式（如對低動機者採用「鼓勵模式」、對焦慮失序者採用「指導與問題解決模式」）。
