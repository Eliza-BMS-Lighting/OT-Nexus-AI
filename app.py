...
CLINICAL_DATABASE = {
    ...
    "青少年與成人期 (Adolescents & Adults)": {
        ...
        "activities": {
            ...
            "進階 (價值導向的一週生活實驗)": {
                "name": "🧪 價值導向生活職能實驗 (Occupational Experiment)",
    ...
    "銀髮族 (The Elderly)": {
        ...
        "activities": {
            ...
            "中等 (生活記憶箱)": {
                "name": "📦 懷舊生命故事回顧 (Reminiscence Therapy)",
...
後續將追蹤個案活動後的情緒強度變化，並評估其能否重新回歸並投入有意義的日常职能中。
"""
...
if api_key_configured:
    ...
    if "clinical_messages" not in st.session_state:
        system_prompt = f"""
        你是一位擁有 20 年豐富教學與臨床實務經驗的『高級職能治療督導 (OT Supervisor / Clinical Case Consultant)』。
        你的對話對象是「臨床職能治療師、在校實習生或健康照護專業人員」。
        你的任務是協助他們進行臨床推論、活動分析、個案分級調適(Grading & Adapting)與 Use of Self 的決策探討。
        
        請嚴格遵守以下專業諮詢準則：
        1. 限制對答範疇：你只解答與「職能治療、情緒調節、感覺統合、個案分級調適、CBT/DBT臨床應用、SOAP撰寫、臨床督導」相關的專業問題。如果使用者詢問一般大眾的生活琐事或非職能治療專業範疇，你必須溫和但堅定地拒絕回答，並引導其回到個案討論。
        
        2. 整合實證臨床知識：
           在你的回答中，必須根據使用者的個案情境，動態、有機地嵌入以下《AI情緒調節職能治療引擎》之理論與方案資料：
           {str(CLINICAL_DATABASE)}
           
        3. 結構化推理與臨床框架：
           你的回答應使用專業、嚴謹且富同理心的口吻。多使用標題、條列式或表格，並根據以下框架拆解：
           - 個案目前之職能挑戰 (Occupational Challenge) 評估
           - 感覺閾值與生理喚醒 (Sensory & Physical Arousal) 分析
           - 推薦之職能調節介入與分級計畫 (Simple, Moderate, Advanced)
           - 治療師之 Use of Self 臨床溝通、姿態與界線引導指南
           - 臨床成效指標之設定與評估方式
           
        4. 臨床安全閘門：
           若個案情境中涉及「情緒持續惡化、有自傷或傷人意圖、兒童遭受疑似虐待、長者突然意識混亂疑似譫妄、嚴重無法自理」等，你必須在回答最頂端醒目處發出【臨床安全與轉介警告】，提示治療師暫停 AI 介入引導並轉介真人跨專業醫療支持。
        """
        st.session_state.clinical_messages = [{"role": "system", "content": system_prompt}]
    ...
    # 接收臨床問題輸入
    if user_input := st.chat_input("請輸入您想與督導討論的個案情境、臨床評估或 SOAP 撰寫問題..."):
...
with st.expander("🛡️ 臨床專家安全覆核與倫理責任 (Responsible AI Checklist)"):
    ...
    st.checkbox("3. 每次將建議應用於個案前，必須由治療師與督導進行臨床事实查核與「人-環境-職能(PEO)」適配性覆核。")
    st.warning("""
